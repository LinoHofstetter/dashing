...
dashing: src/dashing.o $(ALL_ZOBJS) $(DEPS) libzstd.a $(BACKUPOBJ)
    $(CXX) $(CXXFLAGS) $(DBG) $(INCLUDE) $(LD) $(ALL_ZOBJS) $(BACKUPOBJ) -Wl,-e,dashing_main -O3 $< -o $@ $(ZCOMPILE_FLAGS) $(LIB) -march=native -DNDEBUG -lz
...
dashing_d: $(ALL_ZOBJS) $(DEPS) libzstd.a $(DASHINGSRC)
    $(CXX) $(CXXFLAGS) $(DBG) $(INCLUDE) $(LD) $(ALL_ZOBJS) $(DASHINGSRC) -O1 $< -o $@ $(ZCOMPILE_FLAGS) $(LIB) -march=native src/dashing.cpp
